...
for (unsigned int entryCheck = 0; entryCheck < infoDatabase.size(); entryCheck++)
{
if (infoDatabase[entryCheck].dataType == dataType)
entryPresent = true;
}
if (!entryPresent)
{
...
for (unsigned int x = 0; x < infoDatabase.size(); x++)
{
if (dataType == infoDatabase[x].dataType)
for (unsigned int y = 0; y < infoDatabase[x].data.size(); y++)
std::cout << infoDatabase[x].dataType << ":  " << infoDatabase[x].data[y] << "\n";
}
}
*/
